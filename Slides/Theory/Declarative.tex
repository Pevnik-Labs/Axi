...
Well-formed context judgement: $\cart{\Gamma}$
...
\begin{itemize}
  ...
  \item If $\fulltyping{\Gamma}{e}{A}$, then $\cart{\Gamma}$.
  \item If $\proves[\Gamma]{p}{P}$, then $\prop[\Gamma]{P}$.
  \item If $\prop[\Gamma]{P}$, then $\cart[\noncomputable]{\Gamma}$.
  \item If $\type[\Gamma]{A}$, then $\cart[\noncomputable]{\Gamma}$.
  \item If $\cart[\noncomputable]{\Gamma}$, then $\cart[\computable]{\Gamma}$.
\end{itemize}
...
\begin{center}
  $\infrule{}{\cart[\computable]{\emptyctx}}$

  \vspace{2em}

  $\infrule{\cart[\computable]{\Gamma} \quad \type[\cartesianize{\Gamma}]{A} \quad \sidecond{x \notin \Gamma}}{\cart[\computable]{\extend{\Gamma}[r]{x}{A}}}$

  \vspace{2em}

  $\infrule{\cart[\computable]{\Gamma} \quad \fulltyping{\cartesianize{\Gamma}}[\noncomputable]{e}{A} \quad \sidecond{x \notin \Gamma}}{\cart[\computable]{\extend{\Gamma}[r]{x}{A}[e]}}$

  \vspace{2em}

  $\infrule{\cart[\computable]{\Gamma} \quad \prop[\cartesianize{\Gamma}]{P} \quad \sidecond{h \notin \Gamma}}{\cart[\computable]{\assume{\Gamma}{h}{P}}}$
\end{center}
...
\begin{center}
  $\infrule{\cart[\computable]{\Gamma} \quad \sidecond{\Gamma = \cartesianize{\Gamma}}}{\cart[\noncomputable]{\Gamma}}$
\end{center}
...
\begin{center}
  $\infrule{\cart{\Gamma} \quad (x : A) \in \Gamma \quad \subctx{\Gamma}{\varctx{\Gamma}{x}}}{\fulltyping{\Gamma}{x}{A}}$
\end{center}
...
\begin{center}
  $\infrule{\cart{\Gamma} \quad \subctx{\Gamma}{\scalectx{\Zero}{\Gamma}}}{\fulltyping{\Gamma}{\unit}{\Unit}}$
