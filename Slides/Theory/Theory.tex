...
\begin{itemize}
  ...
  \item Computation and conversion checking.
  \item Inductives and records.
  \item Algorithmic system.
  \item Distinguish noncomputable terms.
...
Quantities: \\
$r ::= \Zero \pipe \One \pipe \Unrestricted$

\vspace{2em}

$\Unrestricted$ is the default quantity, so when there's nothing to indicate quantity, it means it's $\Unrestricted$.

\vspace{2em}

Quantities are ordered: \\
\begin{center}
  $\subusage{\Unrestricted}{r}$ \\
  $\subusage{\Zero}{\Zero}$ \\
  $\subusage{\One}{\One}$
\end{center}
...
\begin{center}
  $\addqty{\Zero}{r} = r$ \\
  $\addqty{\Unrestricted}{r} = \Unrestricted$ \\
  $\addqty{\One}{\Zero} = \One$ \\
  $\addqty{\One}{\One} = \Unrestricted$ \\
  $\addqty{\One}{\Unrestricted} = \Unrestricted$
\end{center}
...
\begin{center}
  $\mulqty{\Zero}{r} = \Zero$ \\
  $\mulqty{\One}{r} = r$ \\
  $\mulqty{\Unrestricted}{\Zero} = \Zero$ \\
  $\mulqty{\Unrestricted}{\One} = \Unrestricted$ \\
  $\mulqty{\Unrestricted}{\Unrestricted} = \Unrestricted$ \\
\end{center}
...
Q: Definitions in context look fishy when combined with quantities!
...
\begin{center}
  ...
  %$\infrule[DUBIOUS]{\ctx{\Gamma} \quad \cartesianize{\Gamma'} = \cartesianize{\Gamma} \quad \subctx{\Gamma'}{\scalectx{r}{\Gamma''}} \quad \typing{\Gamma''}{e}{A} \quad \sidecond{x \notin \Gamma}}{\ctx{\extend{\Gamma}[r]{x}{A}[e]}}$

  \vspace{2em}

  $\infrule{\ctx{\Gamma_1} \quad \decomposectx{\Gamma_1}{\Gamma}[r]{\Gamma_2} \quad \typing{\Gamma_2}{e}{A} \quad \sidecond{x \notin \Gamma}}{\ctx{\extend{\Gamma}[r]{x}{A}[e]}}$
...
Q: Where exactly do we need quantity and type annotations?
...
\begin{center}
  ...
  $\infrule{\env{\Sigma} \quad x \notin \Sigma \quad \typing{\Sigma \pipe \emptyctx}{e}{A}}{\env{\extendenv{\Sigma}{x}{A}{e}}}$

  \vspace{2em}

  $\infrule{\env{\Sigma} \quad x \notin \Sigma \quad \typing{\Sigma \pipe \emptyctx}{e}{A} \quad e\ \textbf{fails syntactic check}}{\env{\extendpartial{\Sigma}{x}{A}{e}}}$
