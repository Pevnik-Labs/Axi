...
-- * {0,1,?,+,*} generated by {?}
data Qty
  = -- | Unrestricted (*): zero or more uses, additive and mult. idempotent.
    Un
  | -- | Relevant (+): one or more uses, mult. idempotent, additive absorber.
    Re
  | -- | Affine (?): zero or one uses, multiplicative idempotent.
    Af
...
instance Fractional Qty where
  ...
  _ / Er = Un
  Un / r = complQty r
  r / s = if s <~ r then Li else r

-- | Complete partial order with maximal elements 0 and 1.
(<~) :: Qty -> Qty -> Bool
Un <~ _ = True
Re <~ Li = True
Af <~ Li = True
...
-- | Bounded lattice with the greatest element 0.
(<:) :: Qty -> Qty -> Bool
Un <: _ = True
Re <: Li = True
Af <: Li = True
...
-- | Upper sets according to the complete partial order.
upperQty :: Qty -> [Qty]
upperQty Er = [Er]
upperQty Li = [Li]
...
-- | Meet semilattice compatible with the complete partial order.
infQty :: Qty -> Qty -> Qty
infQty Er Er = Er
infQty r s = abs r * abs s

-- | Join semilattice compatible with the bounded lattice.
supQty :: Qty -> Qty -> Qty
supQty _ Er = Er
supQty Er _ = Er
supQty r Un = r
...
-- | Complement: @r * complQty r = Un@ or @r = complQty r = Er@.
complQty :: Qty -> Qty
complQty Er = Er
complQty Li = Un
complQty Af = Re
