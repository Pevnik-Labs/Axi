...
Some of the less obvious terms: $\outl{e}$ and $\outr{e}$ are projections, $unit$ is the only value of the $Unit$ type and $\elimEmpty{e}$ is the eliminator of the empty type applied to $e$. $\case{e}{e_1}{e_2}$ is the eliminator of the sum type and its arguments $e_1, e_2$ are supposed to be functions. Note that we don't have a type annotation on the lambda, and no type annotations in the language in general. This is not an oversight -- we postpone deciding this detail for later.
...
We will present our language using the machinery of judgements and inference rules. Each kind of \textbf{judgement} represents a meta-level proposition about objects of our language: what is the type of a term, how do terms compute and so on. To make it easier for beginners to distinguish the various kinds of judgement, we will highlight each kind of judgement with a different color.
...
$\computation{e_1}{e_2}{A}$ -- in typing context $\Gamma$, terms $e_1$ and $e_2$ are computationally equal.

\end{frame}

\begin{frame}{Sanity checks}

The inference rules of the system will guarantee that whenever we derive a judgement, then another simpler judgement (called a \textbf{sanity condition}) also holds. For the computational equality $\computation{e_1}{e_2}{A}$ the sanity conditions are $\typing{e_1}{A}$ and $\typing{e_2}{A}$. The typing judgement $\typing{e}{A}$ does not have a sanity condition, but we can think that its sanity condition is the fact that the typing context $\Gamma$ is a well-formed finite partial map from variables to types.

\end{frame}

\begin{frame}{Typing -- basics}

The typing judgement tells us which terms have which types. The most important rule of typing is that variables have whatever type the typing context tells us -- we will call ths the variable rule. Note that the premise of the rule, $\mathcolor{orange}{(x : A) \in \Gamma}$, does not belong to any judgement -- we will call such premises \textbf{side conditions} and highlight them in orange.
...
\begin{center}
  $\infrule[Comp-Refl]{\typing{e}{A}}{\computation{e}{e}{A}}$

  \vspace{2em}

  $\infrule[Comp-Sym]{\computation{e_1}{e_2}{A}}{\computation{e_2}{e_1}{A}}$

  \vspace{2em}

  $\infrule[Comp-Trans]{\computation{e_1}{e_2}{A} \quad \computation{e_2}{e_3}{A}}{\computation{e_1}{e_3}{A}}$
\end{center}
...
\begin{center}
  ...
  $\infrule{}{\computation{\unit}{\unit}{\Unit}}$ \quad
...
\begin{center}
  $\infrule[Valid-Empty]{}{\assctx{\Gamma}{\emptyassctx}}$
...
\begin{center}
  ...
  $\infrule{\prop{\Gamma}{P} \quad \prop{\Gamma}{Q}}{\prop{\Gamma}{\Impl{P}{Q}}}$
...
\begin{center}
  $\infrule[Classic]{\fullholds{\Gamma}{\assume{\Delta}{P}}{R} \quad \fullholds{\Gamma}{\assume{\Delta}{\Not{P}}}{R}}{\holds{R}}$
\end{center}
...
\begin{center}
  ...
  %$\infrule{}{\computation{\btrue}{\btrue}{\Bool}}$ \quad
  %$\infrule{}{\computation{\bfalse}{\bfalse}{\Bool}}$ \quad
  ...
  $\infrule[Comp-If-True]{\typing{e_1}{A} \quad \typing{e_2}{A}}{\computation{\boolElim{\btrue}{e_1}{e_2}}{e_1}{A}}$

  \vspace{1em}

  $\infrule[Comp-If-False]{\typing{e_1}{A} \quad \typing{e_2}{A}}{\computation{\boolElim{\bfalse}{e_1}{e_2}}{e_2}{A}}$
...
\begin{center}
  $\infrule[True-Congr]{}{\computation{\btrue}{\btrue}{\Bool}}$

  \vspace{1em}

  $\infrule[False-Congr]{}{\computation{\bfalse}{\bfalse}{\Bool}}$

  \vspace{1em}

  $\infrule[If-Congr]{\computation{e}{e'}{\Bool} \quad \computation{e_1}{e_1'}{A} \quad \computation{e_2}{e_2'}{A}}{\computation{\boolElim{e}{e_1}{e_2}}{\boolElim{e'}{e_1'}{e_2'}}{A}}$
\end{center}
...
\begin{center}
  $\infrule[Zero-Congr]{}{\computation{\zero}{\zero}{\Nat}}$

  \vspace{1em}

  $\infrule[Succ-Congr]{\computation{n}{n'}{\Nat}}{\computation{\succ[n]}{\succ[n']}{\Nat}}$

  \vspace{1em}

  $\infrule[Nat-Elim-Congr]{\computation{z}{z'}{A} \quad \computation{s}{s'}{\Fun{A}{A}} \quad \computation{n}{n'}{\Nat}}{\computation{\elimNat{z}{s}{n}}{\elimNat{z'}{s'}{n'}}{A}}$
\end{center}
...
\begin{center}
  $\infrule[Nil-Congr]{}{\computation{\nil}{\nil}{\List{A}}}$

  \vspace{1em}

  $\infrule[Cons-Congr]{\computation{h}{h'}{A} \quad \computation{t}{t'}{\List{A}}}{\computation{\cons{h}{t}}{\cons{h'}{t'}}{\List{A}}}$

  \vspace{1em}

  $\infrule[List-Elim-Congr]{\computation{n}{n'}{A} \quad \computation{c}{c'}{\Fun{A}{\Fun{B}{B}}} \quad \computation{l}{l'}{\List{A}}}{\computation{\elimList{n}{c}{l}}{\elimList{n'}{c'}{l'}}{B}}$
\end{center}
...
\begin{center}
  ...
  $\infrule[Scons]{\typing{a}{A} \quad \typing{f}{\Fun{A}{A}}}{\typing{\introStream{a}{f}}{\AxiStream{A}}}$
...
\begin{center}
  $\infrule[Hd-Congr]{\computation{s}{s'}{A}}{\computation{\hd[s]}{\hd[s']}{A}}$

  \vspace{1em}

  $\infrule[Tl-Congr]{\computation{s}{s'}{A}}{\computation{\tl[s]}{\tl[s']}{\AxiStream{A}}}$

  \vspace{1em}

  $\infrule[Scons-Congr]{\computation{a}{a'}{A} \quad \computation{f}{f'}{\Fun{A}{A}}}{\computation{\introStream{a}{f}}{\introStream{a'}{f'}}{\AxiStream{A}}}$
...
The unfortunate reality of product types is that accessing components of nested pairs requires long sequences of $\outl$ and $\outr$, which look ugly. To add insult to injury, product types are not strictly associative, so we need to keep track of parentheses in nested pairs.
...
\begin{center}
  $\infrule[Record-Intro-Congr]{\forall i \in I. \quad \computation{e_i}{e_i'}{A_i}}{\computation{\records{\ell}{e}{i}{I}}{\records{\ell}{e'}{i}{I}}{\recordtypes{\ell}{A}{i}{I}}}$

  \vspace{1em}

  $\infrule[Record-Elim-Congr]{\computation{e}{e'}{\recordtypes{\ell}{A}{i}{I}} \quad \sidecond{j \in I}}{\computation{\proj{\ell_j}{e}}{\proj{\ell_j}{e'}}{A_j}}$
\end{center}
...
\begin{center}
  $\infrule[Variant-Intro-Congr]{\computation{e}{e'}{A_j} \quad \sidecond{j \in I}}{\computation{\app{c_j}{e}}{\app{c_j}{e'}}{\varianttypes{c}{A}{i}{I}}}$

  \vspace{1em}

  $\infrule[Variant-Elim-Congr]{\computation{e}{e'}{\varianttypes{c}{A}{i}{I}} \quad \computation{r}{r'}{\recordtypes{c}{\Fun{A_i}{B}}{i}{I}}}{\computation{\vcase{e}{r}}{\vcase{e'}{r'}}{B}}$
\end{center}
...
\newcommand{\delete}[1][]{\texttt{delete}\ #1}
\newcommand{\create}[1][]{\texttt{create}\ #1}
\newcommand{\id}[1][]{\texttt{id}\ #1}
\newcommand{\comp}{\texttt{comp}}
\newcommand{\swap}[1][]{\texttt{swap}\ #1}
\newcommand{\swop}[1][]{\texttt{swop}\ #1}
\newcommand{\notb}[1][]{\texttt{notb}\ #1}
\newcommand{\andb}{\texttt{andb}}
\newcommand{\add}{\texttt{add}}
\newcommand{\length}[1][]{\texttt{length}\ #1}
\newcommand{\replicate}{\texttt{replicate}}

\begin{frame}{Example programs}

Assume for a moment that $A, B, C$ are arbitrary types. Let's write some programs:

\begin{itemize}
  \item $\delete := \fun{x}{\unit}$
  \item $\create := \fun{x}{\elimEmpty{x}}$
  \item $\id := \fun{x}{x}$
  ...
  \item $\add := \fun{n}{\fun{m}{\elimNat{m}{(\fun{r}{\succ[r]})}{n}}}$
  \item $\length := \fun{l}{\elimList{\zero}{(\fun{h}{\fun{r}{\succ[r]}})}{l}}$
  \item $\replicate := \fun{n}{\fun{x}{\elimNat{\nil}{(\fun{r}{\cons{x}{r}})}{n}}}$
\end{itemize}
...
What are the types of our programs?
...
\begin{itemize}
  \item $\computation{\delete[a]}{\unit}{\Unit}$
  \item $\computation{\id[a]}{a}{A}$
  \item $\computation{\app{\app{\comp}{f}}{\id}}{f}{\Fun{A}{B}}$
  \item $\computation{\app{\app{\comp}{\id}}{g}}{g}{\Fun{B}{C}}$
  \item $\computation{\swap[(\swap[p])]}{p}{\Prod{A}{B}}$
  \item $\computation{\swop[(\swop[(\inl[a])])]}{\inl[a]}{A}$
  \item $\computation{\swop[(\swop[(\inr[b])])]}{\inr[b]}{B}$
  \item $\computation{\notb[(\app{\app{\andb}{\btrue}}{\btrue})]}{\bfalse}{\Bool}$
\end{itemize}
...
\begin{itemize}
  ...
  \item $\holds{\All{a}{\Nat}{\All{b}{\Nat}{\All{c}{\Nat}{\Eq{\app{\app{\add}{(\app{\app{\add}{a}}{b})}}{c}}{\app{\app{\add}{a}}{(\app{\app{\add}{b}}{c})}}}}}}$
  \item $\holds{\All{n}{\Nat}{\All{a}{A}{\Eq{\length[(\app{\app{\replicate}{n}}{a})]}{n}}}}$
\end{itemize}
...
\begin{center}
  ...
  $\infrule[Annot-Congr]{\computation{e}{e'}{A}}{\computation{\annot{e}{A}}{\annot{e'}{A}}{A}}$
\end{center}
...
\begin{center}
  ...
  $\infrule[Var-Ext]{\tyctx{\Gamma} \quad \sidecond{(x : A) \in \Gamma}}{\typing{x}{A}}$ \quad
  $\infrule[Var-Def]{\tyctx{\Gamma} \quad \sidecond{(x : A := e) \in \Gamma}}{\typing{x}{A}}$

  \vspace{1em}

  $\infrule[Comp-Var]{\tyctx{\Gamma} \quad \sidecond{(x : A := e) \in \Gamma}}{\computation{x}{e}{A}}$
\end{center}
...
\begin{center}
  $\infrule[App-Lam-Def]{\fullcomputation{\extenddef{\Gamma}{x}{A}{a}}{b}{b'}{B}}{\computation{\app{(\fun{x}{b})}{a}}{b'}{B}}$
...
\begin{center}
  $\infrule[Ctx-Extend]{\tyctx{\Gamma} \quad \type{A} \quad \sidecond{x \notin \Gamma}}{\tyctx{\extend{\Gamma}{x}{A}}}$
\end{center}

With such a setup, $\tyctx{\Gamma}$ is a sanity condition for $\type{A}$, whereas $\type{A}$ is a sanity condition for $\typing{x}{A}$.
...
\begin{center}
  ...
  $\infrule[Refinement-Intro]{\fulltyping{\Gamma \pipe \Delta}{e}{A} \quad \holds{\subst{P}{x}{e}}}{\fulltyping{\Gamma \pipe \Delta}{\refine{e}}{\refinement{x}{A}{P}}}$
...
\begin{center}
  ...
  $\infrule[Quotient-Intro]{\fulltyping{\Gamma \pipe \Delta}{e}{A} \quad \prop{\extend{\extend{\Gamma}{x}{A}}{y}{A}}{R} \quad \mathcolor{red}{\texttt{Equivalence}\ R}}{\fulltyping{\Gamma \pipe \Delta}{\qin{e}}{\quotient{A}{R}}}$

  \vspace{1em}

  $\infrule[Quotient-Elim]{\fulltyping{\Gamma \pipe \Delta}{e}{\quotient{A}{R}} \quad \fulltyping{\Gamma \pipe \Delta}{f}{\Fun{A}{B}} \quad \mathcolor{red}{f\ \texttt{preserves}\ R}}{\fulltyping{\Gamma \pipe \Delta}{\qout{f}{e}}{B}}$

  \vspace{1em}

  $\infrule[Quotient-Comp]{\fulltyping{\Gamma \pipe \Delta}{e}{A} \quad \fulltyping{\Gamma \pipe \Delta}{f}{\Fun{A}{B}} \quad \mathcolor{red}{f\ \texttt{preserves}\ R}}{\computation{\qout{f}{(\qin{e})}}{\app{f}{e}}{B}}$
\end{center}
