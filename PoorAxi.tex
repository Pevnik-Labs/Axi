...
\newcommand{\context}[1]{#1\ \texttt{ctx}}
...
%$\context{\Gamma}$ -- $\Gamma$ is a well-formed typing context.

\begin{center}
  $\infrule{}{\context{\emptyctx}}$ \quad
  $\infrule{\context{\Gamma} \quad x \notin \Gamma}{\context{\Gamma, x : A}}$ \quad
  $\infrule{\context{\Gamma} \quad \typing{\Gamma}{e}{A} \quad x \notin \Gamma}{\context{\Gamma, \cdef{x}{A}{e}}}$

  \vspace{1em}

  $\infrule{\context{\Gamma, \cdef{x}{A}{e}}}{\computation{\Gamma, \cdef{x}{A}{e}}{x}{e}{A}}$
\end{center}
...
\begin{center}
  $\infrule{\computation{\Gamma, \cdef{x}{A}{e_1}}{e_2}{e_2'}{B}}{\computation{\Gamma}{\letin{x}{e_1}{e_2}}{e_2'}{B}}$
\end{center}
...
\begin{center}
  ...
  $\infrule{\typing{\Gamma}{e}{R} \quad (\ctype{\ell}{A}) \in R}{\typing{\Gamma}{\proj{\ell}{e}}{A}}$

  \vspace{1em}

  $\infrule{\typing{\Gamma}{\record{\fields{\ell}{e}{i}}}{\recordtype{\ctype{\ell_i}{A_i}}} \quad \exists i. \ell = \ell_i}{\computation{\Gamma}{\proj{l}{\record{\fields{\ell}{e}{i}}}}{e_i}{A_i}}$
...
For a moment, let's say that types can depend on propositions. This means that we have a well-formed type judgement which depends both on typing contexts and assumption contexts. We can now express refinement types.

\vspace{1em}

\begin{center}
  $\infrule{\Gamma \vdash A\ \texttt{type} \quad \prop{\Gamma, \ctype{x}{A}}{P}}{\Gamma \vdash \refinement{x}{A}{P}\ \texttt{type}}$
  ...
  $\infrule{\typing{\Gamma \pipe \Delta}{e}{\refinement{x}{A}{P}}}{\typing{\Gamma \pipe \Delta}{\texttt{proj}\ e}{A}}$ \quad
  $\infrule{\typing{\Gamma \pipe \Delta}{e}{\refinement{x}{A}{P}}}{\holds{\Gamma}{\Delta}{\subst{P}{x}{e}}}$
...
\begin{itemize}
  ...
  \item Subtyping -- some types are now in a subtyping relation $A :< B$ and an $a : A$ can be used in every place where a term of type $B$ is expected.
\end{itemize}
