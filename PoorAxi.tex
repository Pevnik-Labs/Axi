...
\newcommand{\assctx}[2]{\mathcolor{cyan}{#1\ \vdash #2\ \texttt{valid}}}
...
\begin{center}
  $\infrule{}{\typing{\true}{\Bool}}$ \quad
  $\infrule{}{\typing{\false}{\Bool}}$
...
\begin{center}
  $\infrule[True-Congr]{}{\computation{\true}{\true}{\Bool}}$

  \vspace{1em}

  $\infrule[False-Congr]{}{\computation{\false}{\false}{\Bool}}$
...
\begin{itemize}
  ...
  \item $\notb := \fun{b}{\boolElim{b}{\false}{\true}}$
  \item $\andb := \fun{x}{\fun{y}{\boolElim{x}{y}{\false}}}$
...
\begin{itemize}
  ...
  \item $\computation{\notb[(\app{\app{\andb}{\true}}{\true})]}{\false}{\Bool}$
\end{itemize}
...
Types of variables can be inferred, because they appear in the context. Type annotations are directives to infer the type from the annotation, provided that the term can be checked to have this type. There's also a subsumption rule, which is applied when no other checking rule applies.
