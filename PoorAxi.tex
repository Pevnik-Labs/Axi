...
Some of the less obvious terms: $\outl{e}$ and $\outr{e}$ are projections, $unit$ is the only varlue of the $Unit$ type and $\elimEmpty{e}$ is the eliminator of the empty type applied to $e$. $\case{e}{e_1}{e_2}$ is the eliminator of the sum type and its arguments $e_1, e_2$ are supposed to be functions.
...
$\infrule{\mathcal{J}_1 \dots \mathcal{J}_n}{\mathcal{J}}$

\vspace{1em}

where the premises (the judgements $\mathcal{J}_i$) are written above the bar and the conclusion (the judgement $\mathcal{J}$) is written below the bar. A rule can have many premises, but only a single conclusion. Most of the time it is best to read an inference rule upside down, i.e. to first read the conclusion and only then the premises.

\end{frame}

\section{Typing}

\begin{frame}{Judgements -- programming language}
...
The typing judgement tells us which terms have which types. The most important rule of typing is that variables have whatever type the typing context tells us. Note that the premise of the rule, $\mathcolor{orange}{(x : A) \in \Gamma}$, does not belong to any judgement -- we will call such premises \textcolor{orange}{side conditions}.
...
\begin{center}
  $\infrule[True-Congr]{\tyctx{\Gamma}}{\computation{\true}{\true}{\Bool}}$

  \vspace{1em}

  $\infrule[False-Congr]{\tyctx{\Gamma}}{\computation{\false}{\false}{\Bool}}$

  \vspace{1em}

  $\infrule[Bool-Elim-Congr]{\computation{e}{e'}{\Bool} \quad \computation{e_1}{e_1'}{A} \quad \computation{e_2}{e_2'}{A}}{\computation{\boolElim{e}{e_1}{e_2}}{\boolElim{e'}{e_1'}{e_2'}}{A}}$
\end{center}
...
\begin{center}
  $\infrule[Zero-Congr]{\tyctx{\Gamma}}{\computation{\zero}{\zero}{\Nat}}$
...
\begin{center}
  $\infrule[Nil-Congr]{\tyctx{\Gamma}}{\computation{\nil}{\nil}{\List{A}}}$
...
\begin{center}
  ...
  $\infrule[Stream-Intro-Congr]{\computation{a}{a'}{A} \quad \computation{f}{f'}{\Fun{A}{A}}}{\computation{\introStream{a}{f}}{\introStream{a'}{f'}}{\AxiStream{A}}}$
...
\begin{frame}{Definitions in context}
...
\begin{frame}{Definitions in context -- practical consequences}
