...
proof
  ...
  . unfold injective
    ...
    chaining
      === x
      === g (f x)   by (rewrite <-(instantiate gf with x) in refl) // Note that we need `x === g (f x)` whereas `gf` gives `g (f x) === x`.
      === g (f y)   by eq
...
proof
  ...
  . both
    . pick-any a' : A                                   // g (f a') === a'
      unfold g                                          // (\ b -> pick-witness a for (instantiate sur with b) in a) (f a') === a'
    ...
    . pick-any b : B                                    // f (g b) === b
      unfold g                                          // f ((\ (b : B) -> pick-witness a for (instantiate sur with b) in a) b) === b
...
proof
  ...
  . pick-any a : A                                 // Γ, a : A |- B
    pick-witness b for (instantiate allex with a)  // Γ, a : A, b : B, R a b |- B
    b
  . pick-any a : A                                 // Γ, a : A |- R a (f a)
    unfold f                                       // Γ, a : A |- R a ((\ (a : A) -> pick-witness b for (instantiate allex with a) in b) a)
