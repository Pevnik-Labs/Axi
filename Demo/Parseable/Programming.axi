...
proof
  ...
  | cons h (t & IH) =>
    chaining
      === map (fun a => g (f a)) (cons h t)
      === cons (g (f h)) (map (fun a => g (f a)) t)  by simpl
      === cons (g (f h)) (map g (map f t))           by rewrite IH
      === map g (map f (cons h t))                   by refl
qed
...
proof
  ...
  | nil =>
    chaining
      === app (app nil l2) l3
      === app nil (app l2 l3)  by refl
  | cons h (t & IH) =>
    chaining
      === app (app (cons h t) l2) l3
      === cons h (app (app t l2) l3)  by simpl
      === cons h (app t (app l2 l3))  by rewrite IH
      === app (cons h t) (app l2 l3)  by refl
qed
...
class LawfulEq A <= Eq A where
  eqb-spec1 : forall x y : A, eqb x y === true --> x === y
  eqb-spec2 : forall x y : A, x === y --> eqb x y === true
...
proof
  ...
  | cons h (t & IH) =>
    chaining
      ...
      === andb (eqb h h) (eqb t t)   by simpl
      === andb true (eqb t t)        by rewrite (apply (instantiate eqb-spec2 with h h) refl)
      === andb true true             by rewrite IH
      === true                       by refl
qed
...
instance LawfulEq (List A) <= LawfulEq A where
  theorem eqb-spec1 :
    forall l1 l2 : List A,
      eqb l1 l2 === true --> l1 === l2
  proof
    pick-any l1 l2
    assume h : eqb l1 l2 === true
    induction l1 with
    | nil =>
      cases l2 with
      | nil =>
    ...
    | cons h1 t1 =>
      ...
      | nil =>
        ...
      | cons h2 t2 =>
        // h : andb (eqb h1 h2) (eqb t1 t2) === true
  qed

  theorem eqb-spec2 :
    forall l1 l2 : List A,
      l1 === l2 --> eqb l1 l2 === true
  proof
    pick-any A _ l1 l2
    assume h : l1 === l2
    chaining
      ...
      === true       by rewrite (instantiate eqb-diag with A ? l2)
  qed
...
theorem neqb-spec :
  forall {A} [LawfulEq A] (x y : A),
    neqb x y === true <--> ~ (x === y)
proof
  pick-any A _ x y    // A : Type, LawfulEq A, x y : A |- neqb x y === true <--> ~ (x === y)
  unfold neqb         // A : Type, LawfulEq A, x y : A |- notb (eqb x y) === true <--> ~ (x === y)
  both
  . assume (hnot : notb (eqb x y) === true) (heq : x === y)
                      // A : Type, LawfulEq A, x y : A, hnot : notb (eqb x y) === true, heq : x === y |- False
qed