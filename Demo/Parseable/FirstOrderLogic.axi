...
proof
  ...
  . assume (exists x such-that exor)  // x : A, exor : P x \/ Q x |- (exists x : A, P x) \/ (exists x : A, Q x)
    ...
    . assume p                        // x : A, exor : P x \/ Q x, p : P x |- (exists x : A, P x) \/ (exists x : A, Q x)
      ...
      exists x                        // x : A, exor : P x \/ Q x, p : P x |- P x
      assumption                      // Goal solved!
    ...
    . assume q                        // x : A, exor : P x \/ Q x, q : Q x |- (exists x : A, P x) \/ (exists x : A, Q x)
      ...
      exists x                        // x : A, exor : P x \/ Q x, q : Q x |- Q x
      assumption                      // Goal solved!
  ...
  . assume orex                       // orex : (exists x : A, P x) \/ (exists x : A, Q x) |- exists x : A, P x \/ Q x
    ...
    . assume (exists x such-that p)   // orex : (exists x : A, P x) \/ (exists x : A, Q x), x : A, p : P x |- exists x : A, P x \/ Q x
      exists x                        // orex : (exists x : A, P x) \/ (exists x : A, Q x), x : A, p : P x |- P x \/ Q x
    ...
    . assume (exists x such-that q)   // orex : (exists x : A, P x) \/ (exists x : A, Q x), x : A, q : Q x |- exists x : A, P x \/ Q x
      exists x                        // orex : (exists x : A, P x) \/ (exists x : A, Q x), x : A, q : Q x |- P x \/ Q x
...
proof
  ...
  .                                      // |- ~ (exists x : A, P x) --> (forall x : A, ~ P x)
    ...
    exists x                             // nex : ~ (exists x : A, P x), x : A, p : P x |- P x
    assumption                           // Goal solved!
  .                                      // |- (forall x : A, ~ P x) --> ~ (exists x : A, P x)
    assume allnp (exists x such-that p)  // allnp : forall x : A, ~ P x, x : A, p : P x |- False
...
proof
  ...
  .                                                // |- ~ (forall x : A, P x) --> exists x : A, ~ P x
    ...
    exists x                                       // nall : ~ (forall x : A, P x), nexn : ~ (exists x : A, ~ P x), x : A, np : ~ P x |- ~ P x
    assumption                                     // Goal solved!
  .                                                // |- (exists x : A, ~ P x) --> ~ (forall x : A, P x)
    assume (exists x such-that np) all             // x : A, np : ~ P x, all : forall x : A, P x |- False
