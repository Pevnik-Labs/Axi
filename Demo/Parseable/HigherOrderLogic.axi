...
proof
  ...
  . assume mx                                        // A : Type, P : A -> Prop, ex : (forall R : Prop, (forall x : A, P x --> R) --> R) |- exists x : A, P x
    ...
    exists x                                         // A : Type, P : A -> Prop, ex : (forall R : Prop, (forall x : A, P x --> R) --> R), x : A, px : P x |- P x
    assumption                                       // Goal solved!
                                                     // A : Type, P : A -> Prop |- (exists x : A, P x) --> forall R : Prop, (forall x : A, P x --> R) --> R
  . assume (exists x such-that px)                   // A : Type, P : A -> Prop, x : A px : P x |- forall R : Prop, (forall x : A, P x --> R) --> R
...
proof
  ...
  . assume all                                             // A : Type, x y : A, all : (forall P : A -> Prop, P x --> P y) |- x === y
    apply (instantiate all with (fun (a : A) => x === a))  // A : Type, x y : A, all : (forall P : A -> Prop, P x --> P y) |- x === x
    refl                                                   // Goal solved!
