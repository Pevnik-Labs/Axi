...
\begin{frame}{Let logic guide us}

I think that the principle guiding this choice should be logic, because we want to prove theorems about our programs. Proving theorems about machine code or a low-level imperative assembly language is not TODO
...
\begin{itemize}
  \item A type-checking-like component. For example, to check that $\textbf{both}\ e_1\ e_2$ proves $P \land Q$, we need to check that $e_1$ proves $P$ and that $e_2$ proves $Q$. This is basically the same story as type checking pairs: to check that $(t_1, t_2)$ has type $A \times B$, we need to check that $t_1$ has type $A$ and that $t_2$ has type $B$. This is the Curry-Howard correspondence at work.
...
I don't have a definite answer to the question of whether proof checking should be paid, but the names I gave to the solutions suggest that TODO
